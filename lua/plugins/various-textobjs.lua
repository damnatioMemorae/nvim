return {
        ...
        keys  = {
                ...
                -- { "in", function() require("vim.treesitter._select").select_prev(vim.v.count1) end, mode = "x", desc = "lect previous node" },
                -- { "an", function() require("vim.treesitter._select").select_next(vim.v.count1) end, mode = "x", desc = "lect next node" },
                ...
                { -- delete surrounding indentation
                        ...
                        function()
                                ...
                                local indentation_found = vim.fn.mode() == "V"
                                ...
                                vim.cmd(end_border_ln .. " delete")
                                vim.cmd(start_border_ln .. " delete")

                                vim.defer_fn(function() vim.api.nvim_win_set_cursor(0, cursor_before) end, 1)
                        end,
                ...
                { -- yank surrounding inner indentation
                        ...
                        function()
                                ...
                                local end_ln     = vim.api.nvim_buf_get_mark(0, ">")[1] - 1
                                local start_line = vim.api.nvim_buf_get_lines(0, start_ln, start_ln + 1, false)[1]
                                local end_line   = vim.api.nvim_buf_get_lines(0, end_ln, end_ln + 1, false)[1]
                                vim.fn.setreg("+", start_line .. "\n" .. end_line .. "\n")

                                local dur   = 1500
                                local ns    = vim.api.nvim_create_namespace("ysii")
                                local bufnr = vim.api.nvim_get_current_buf()
                                vim.hl.range(bufnr, ns, "IncSearch", { start_ln, 0 }, { start_ln, -1 }, { timeout = dur })
                                vim.hl.range(bufnr, ns, "IncSearch", { end_ln, 0 },   { end_ln, -1 },   { timeout = dur })
                        end,
                        desc = "Delete surrounding indent",
                },
