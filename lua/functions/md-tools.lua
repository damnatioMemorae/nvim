...
local function wrap(startWrap, endWrap)
        ...
        if startWrap == "mdlink" then
                ...
                clipboard_url = clipb:match "^#[%w-]+$"
                    or clipb:match [[^%l%l%l+://[^%s)%]}"'`>]+]]
                    or ""
...
local function autoBullet(key)
        ...
        elseif key == "<CR>" and not empty_list then
                ...
                if vim.startswith(after_cur, continued) then continued = "" end
...
local function followMdlinkOrWikilink()
        ...
        while true do
                ...
                if not (mdlink_end or wiki_end or url_end) then break end
        ...
        match(type) {
                list = function()
                        updated = cur_line:gsub("^(%s*)([%d.*+-]+ )", function(indent, list)
                                ...
                                if list:find "[*+-] " then return indent .. "1. " end
                                if list:find "%d+%. " then return indent end
                                return indent .. list
                        end)
                        if updated == cur_line then
                                updated = cur_line
                                    :gsub("^(%s*)[*+-] %[[ x-]%] ", "%1")
                                    :gsub("^#+ ", "")
                                    :gsub("^(%s*)(.*)", "%1- %2")
                ...
                _    = function() error(("Unknown type for `.cycle()`: `%s`"):format(type)) end,
        }
        -- if type == "list" then
        --         updated = cur_line:gsub("^(%s*)([%d.*+-]+ )", function(indent, list)
        --                 local is_task = cur_line:find "^%s*[*+-] %[[ x-]%] "
...
local function codeBlockFromClipboard()
...
local function getTitleForUrl(url)
        ...
        net.request(url, {}, vim.schedule_wrap(function(err, out)
...
---@param reg '"'|"+"|string
---@return nil
local function addTitleToUrlIfMarkdown(reg)
        ...
        if char_under_cursor:find "[()<>]" then return end
        local clipb = fn.getreg(reg)
        local url   = clipb:match "^%l+://%S+$"
...
return {
        ...
        followMdlinkOrWikilink  = followMdlinkOrWikilink,
        cycle                   = cycle,
        codeBlockFromClipboard  = codeBlockFromClipboard,
        addTitleToUrl           = addTitleToUrl,
        addTitleToUrlIfMarkdown = addTitleToUrlIfMarkdown,
}
